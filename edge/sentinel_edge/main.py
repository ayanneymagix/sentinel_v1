...
# Automatically add edge root to sys.path so running directly works from any folder
...
from sentinel_edge.camera.stream_manager import StreamManager, CameraDescriptor, FramePacket
...
def build_event(
    ...
    for det in detections:
        rl = det.get("risk_level")
        if rl == "CRITICAL":
            risk_level = "CRITICAL"
            risk_reason = det.get("risk_reason", "")
            break
        elif rl == "HIGH" and risk_level != "CRITICAL":
            risk_level = "HIGH"
            risk_reason = det.get("risk_reason", "")
        elif rl == "MEDIUM" and risk_level not in ("CRITICAL", "HIGH"):
            risk_level = "MEDIUM"
            risk_reason = det.get("risk_reason", "")
    ...
    metadata = {
        ...
        "source": str(getattr(camera, "source", getattr(camera, "rtsp_url", ""))),
        "tracking": {
    ...
    if video_time_seconds is not None:
        metadata["video_time_seconds"] = round(video_time_seconds, 3)
    ...
    return {
        ...
        "video_time_seconds": round(video_time_seconds, 3) if video_time_seconds is not None else None,
...
def _execute_alpr_task(
    ...
    """Execute ALPR & threat intelligence in a non-blocking background worker."""
    try:
        execution_result = execution_router.route(execution_request)
        if execution_result.success:
            execution_counts["successful"] += 1
        else:
            execution_counts["failed"] += 1
            logger.warning(
                "[%s] AI execution failed | track=%s | target=%s | node=%s | error=%s",
                camera.id,
                track_id,
        ...
        logger.info(
            "[%s] AI execution completed | track=%s | target=%s | executor=%s | latency=%.2fms",
            camera.id,
            track_id,
        ...
        class _ALPRExecutionView:
            def __init__(self, payload: dict):
                ...
                self.plate_confidence = float(payload.get("plate_confidence", 0.0))
                self.detector_confidence = float(payload.get("detector_confidence", 0.0))
                self.recognized = bool(payload.get("recognized", False))
        ...
        if plate_key not in emitted_plates:
                ...
                detections=[
                    {
                        ...
                        "track_id": track_id,
                        "confidence": float(result.plate_confidence),
                    }
                ],
                frame_number=frame_number,
            ...
            try:
                if event_client.send(event):
                    emitted_plates.add(plate_key)
                    logger.info(
                        "[%s] ALPR event sent | track=%s | plate=%s | confidence=%.3f",
                        camera.id,
                        track_id,
                        plate,
                ...
                logger.exception("[%s] Failed to send ALPR event | track=%s | plate=%s", camera.id, track_id, plate)

        if threat_intel_engine is None:
            return

        try:
            assessment = threat_intel_engine.assess(plate)
        except Exception:
            logger.exception("[%s] Threat intelligence assessment failed | track=%s | plate=%s", camera.id, track_id, plate)
            return

        behavior_events = behavior_events_by_track.get(track_id, [])
        behavior_severity = None
        severity_rank = {"LOW": 1, "MEDIUM": 2, "HIGH": 3, "CRITICAL": 4}
        for behavior_event in behavior_events:
            severity = str(getattr(behavior_event, "severity", "")).upper()
            if not behavior_severity or severity_rank.get(severity, 0) > severity_rank.get(behavior_severity, 0):
                behavior_severity = severity

        context_assessment = context_fusion_engine.assess(
            track_id=track_id,
            threat_risk=float(assessment.risk_score),
        ...
        threat_key = (track_id, assessment.plate, assessment.threat_type.value)
        ...
        threat_event = build_event(
            ...
            metadata_extra={
                ...
                "threat_intelligence": assessment.to_dict(),
                "context_assessment": context_assessment.to_dict(),
            },
        )
        try:
            if event_client.send(threat_event):
                emitted_threats.add(threat_key)
                logger.warning(
                    "[%s] THREAT DETECTED | track=%s | plate=%s | type=%s | severity=%s | risk=%.3f",
                    camera.id,
                    track_id,
                    assessment.plate,
            ...
            logger.exception("[%s] Failed to send threat event | track=%s | plate=%s", camera.id, track_id, assessment.plate)

    except Exception:
        logger.exception("[%s] Asynchronous ALPR task error | track=%s", camera.id, track_id)
...
def process_alpr(
    ...
    emitted_plates,
    emitted_threats,
    alpr_attempted_tracks=None,
    alpr_executor=None,
):
    """Run ALPR and Threat Intelligence for selected vehicle tracks without blocking frame ingestion."""
    ...
    for detection in enriched_detections:
        ...
        # Skip non-vehicles (pedestrians, bicycles do not have license plates)
        if object_type not in {
            "car",
            "truck",
            "bus",
        ...
        # Deduplicate: only attempt ALPR once per track ID to preserve realtime frame throughput
        if alpr_attempted_tracks is not None:
            if track_id in alpr_attempted_tracks:
                continue
            alpr_attempted_tracks.add(track_id)
        ...
        offload_decision = offload_decisions_by_track.get(track_id)
        ...
        if not offload_decision.accepted:
            logger.warning(
                "[%s] AI workload rejected by scheduler | track=%s | reason=%s",
                camera.id,
                track_id,
        ...
        if alpr_executor is not None:
            alpr_executor.submit(_execute_alpr_task, *task_args)
        else:
            _execute_alpr_task(*task_args)
...
def build_alpr_engine(config):
    ...
    try:
        model_path = alpr.plate_model_path
        if not os.path.isabs(model_path) and not os.path.exists(model_path):
            candidate = os.path.join(_edge_root, model_path.lstrip("./"))
...
def process_camera(
    ...
    logger.info(
        "[%s] Starting camera: %s",
        camera.id,
        getattr(camera, "source", getattr(camera, "rtsp_url", "")),
    )
    ...
    # --------------------------------------------------------
    # Camera StreamManager Registration
    # --------------------------------------------------------

    stream_manager = StreamManager(
        on_discontinuity=lambda cid, gen: on_discontinuity()
    )

    if isinstance(camera, CameraDescriptor):
        descriptor = camera
    else:
        descriptor = CameraDescriptor(
            id=camera.id,
            name=getattr(camera, "name", camera.id),
            rtsp_url=getattr(camera, "rtsp_url", getattr(camera, "source", "")),
            codec=getattr(camera, "codec", "h264"),
            live=getattr(camera, "live", True),
        )

    stream_manager.register_camera(descriptor)
    stream_manager.start_camera(descriptor.id)
    ...
    emitted_plates = set()
    emitted_threats = set()
    alpr_attempted_tracks = set()
    alpr_executor = concurrent.futures.ThreadPoolExecutor(max_workers=2, thread_name_prefix=f"{camera.id}-alpr")
    ...
    try:
        ...
        while True:
            ...
            # ------------------------------------------------
            # Read FramePacket from StreamManager (authoritative PTS)
            # ------------------------------------------------

            packet = stream_manager.get_frame(camera.id, timeout=2.0)

            if packet is None:
                health = stream_manager.get_health(camera.id)
                if health.get("stream_state") in ("OFFLINE", "STOPPED"):
                    logger.info("[%s] Stream offline/stopped", camera.id)
                    break
                time.sleep(0.01)
                continue
...
# =========================================================
    ...
    alpr_attempted_tracks=alpr_attempted_tracks,
    alpr_executor=alpr_executor,
)
            ...
            # Maintain state_manager rate-limiter updates
            for detection in enriched_detections:
                track_id = detection.get("track_id")
                if track_id is not None:
                    state_manager.should_emit_update(track_id, timestamp)

            # For real-time visual CCTV tracking overlay, emit all active frame detections to eliminate flickering
            emit_detections = [
                detection for detection in enriched_detections
                if detection.get("bbox") is not None or detection.get("norm_bbox") is not None
            ]
        ...
        alpr_attempted_tracks.clear()
        alpr_executor.shutdown(wait=False)
...
def main():
    ...
    try:
        # --------------------------------------------------------
        # Dynamic CCTV Camera Discovery (Authoritative /cameras.json)
        # --------------------------------------------------------
        gateway_url = getattr(config.dynamic_ingest, "gateway_url", "https://cctv.corp8.cloud") if hasattr(config, "dynamic_ingest") else "https://cctv.corp8.cloud"
        try:
            discovered_cameras = discover_camera_topology(gateway_url)
            logger.info("Dynamic CCTV discovery: %d cameras discovered from %s", len(discovered_cameras), gateway_url)
        except Exception as exc:
            logger.warning("Dynamic CCTV discovery notice: %s. Using configured cameras.", exc)
            discovered_cameras = []

        # Dynamic camera ingestion from discovery
        enabled_cameras = [c for c in discovered_cameras if c.live]
        if not enabled_cameras and discovered_cameras:
            enabled_cameras = list(discovered_cameras)
        if not enabled_cameras:
                    ...
                    if cam_cfg.enabled:
                        enabled_cameras.append(
                            CameraDescriptor(
                                id=cam_cfg.id,
                                name=cam_cfg.name,
                                rtsp_url=getattr(cam_cfg, "source", ""),
                                fallback_file=f"data/{cam_cfg.id}.mp4",
                                codec="h264",
                                live=True,
                            )
                        )
            if not enabled_cameras:
                data_dir = Path("data")
                if data_dir.exists():
                    for vid_file in sorted(data_dir.glob("*.mp4")):
                        stem = vid_file.stem
                        ...
                        enabled_cameras.append(
                            CameraDescriptor(
                                ...
                                name=clean_name,
                                rtsp_url="",
                                fallback_file=str(vid_file).replace("\\", "/"),
                                codec="h264",
                                live=True,
                            )
                        )
        ...
        if len(enabled_cameras) <= 1:
            for camera in enabled_cameras:
                tracker = VehicleTracker(
                    ...
                    camera_id=camera.id,
                    camera_name=getattr(camera, "name", camera.id),
                )
                process_camera(
                    camera=camera,
                    config=config,
