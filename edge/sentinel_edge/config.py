...
# ============================================================
# DYNAMIC INGEST CONFIGURATION
# ============================================================

class DynamicIngestSettings(BaseModel):
    enabled: bool = True
    gateway_url: str = "https://cctv.corp8.cloud"


# ============================================================
# CAMERA CONFIGURATION (OPTIONAL / LEGACY FALLBACK)
# ============================================================
...
class SentinelConfig(BaseModel):
    ...
    dynamic_ingest: DynamicIngestSettings = DynamicIngestSettings()
...
def load_config(
    ...
    if not config_path.exists():
        # Fallback to search relative to edge root folder
        edge_root = Path(__file__).resolve().parent.parent
        alt_path = edge_root / path
        if alt_path.exists():
            config_path = alt_path
        elif (edge_root / "config" / "node.yaml").exists():
            config_path = edge_root / "config" / "node.yaml"
        else:
