...
def test_temporal_voting_selects_consistent_plate():
    ...
    plate, confidence, votes, is_finalized = aggregator.best(10)
...
def test_temporal_voting_requires_minimum_votes():
    ...
    plate, confidence, votes, is_finalized = aggregator.best(20)
    assert not is_finalized
    assert votes == 1
    assert aggregator.get_finalized(20) is None
