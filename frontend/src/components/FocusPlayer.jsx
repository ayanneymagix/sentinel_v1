 ...
 * Endpoint: Authoritative WHEP media gateway endpoint
 * 
 * Features:
 ...
 * - Graceful fallback to HLS live stream if WHEP media server is unreachable.
 */
export default function FocusPlayer({ camera, onClose = () => {} }) {
  ...
  const [connectionState, setConnectionState] = useState('CONNECTING'); // 'CONNECTING' | 'LIVE' | 'DEGRADED' | 'FAILED'
  const [latencyMs, setLatencyMs] = useState(null);
  const [streamStats, setStreamStats] = useState({ fps: null, resolution: null, bitrate: null });
  ...
  // Authoritative Gateway Endpoints (Zero RTSP credentials)
  const whepUrl = camera?.whep_url || camera?.endpoints?.whep || (camId ? `/api/cameras/${camId}/whep` : '');
  const hlsUrl = (camera?.hls_url && !camera.hls_url.includes('cctv.corp8.cloud')) ? camera.hls_url : `/api/cameras/${camId}/hls/index.m3u8`;
  ...
  useEffect(() => {
    ...
    async function initWhepSession() {
      try {
        ...
        console.warn(`[WHEP] Session notice for ${camId}:`, err.message);
        if (!isCancelled) {
          // Fallback to authoritative HLS endpoint
    ...
    // RTT & WebRTC stats polling
    let prevBytes = 0;
    let prevTime = Date.now();
    const statsTimer = setInterval(async () => {
      if (pcRef.current && pcRef.current.getStats) {
        try {
          ...
          stats.forEach((report) => {
            ...
            if (report.type === 'inbound-rtp' && report.kind === 'video') {
              ...
              if (report.framesPerSecond != null) {
                setStreamStats((prev) => ({ ...prev, fps: Math.round(report.framesPerSecond) }));
              }
              if (report.bytesReceived != null && dt > 0 && prevBytes > 0) {
                const bps = ((report.bytesReceived - prevBytes) * 8) / dt;
                const mbps = (bps / 1000000).toFixed(1);
                setStreamStats((prev) => ({ ...prev, bitrate: `${mbps} Mbps` }));
              }
              prevBytes = report.bytesReceived || prevBytes;
              prevTime = now;
            }
          });
        } catch (_) {}
      ...
      >
        ...
        >
          <div style={{ display: 'flex', gap: '24px' }}>
            <span>Framerate: <b style={{ color: '#38bdf8' }}>{streamStats.fps != null ? `${streamStats.fps} FPS` : 'UNAVAILABLE'}</b></span>
            <span>Bitrate: <b style={{ color: '#f59e0b' }}>{streamStats.bitrate || 'UNAVAILABLE'}</b></span>
